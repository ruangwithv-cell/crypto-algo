...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Daily shadow PnL tracker for long sleeve scenarios")
    p.add_argument("--data-json", type=Path, required=True)
    p.add_argument("--long-sleeve-json", type=Path, required=True, help="Latest long sleeve picks json")
...
def _load_long_weights(path: Path) -> dict[str, float]:
    ...
    with args.csv_path.open("a", newline="") as f:
        ...
        w.writerow(
            [
                ...
                f"{funding_pnl:.8f}",
                len(last_weights),
                f"{sum(abs(float(v)) for v in last_weights.values()):.6f}",
                len(cur_weights),
                f"{sum(abs(float(v)) for v in cur_weights.values()):.6f}",
                used,
            ]
        )
